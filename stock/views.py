...
def search_stock(base_url, ticker_symbol):
    API_token = settings.IEX_API_TOKEN
    url = base_url + ticker_symbol + '/quote?token='+ API_token
    try:
...
def search_stock_batch(base_url, stock_tickers):
    data_list = []
    token = settings.IEX_API_TOKEN
    url = base_url + stock_tickers + '/quote?token=' + token
    try:
